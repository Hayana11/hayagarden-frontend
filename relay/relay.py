...
DEFAULT_CHROME_BIN = "/snap/bin/chromium"
PROFILE_DIR = Path("/root/snap/chromium/common/hayagarden-browser-base-profile")
SCREENCAST_QUALITY = 85
_PASS = os.environ.get("BROWSER_RELAY_PASS", "")
_PROXY_SERVER = os.environ.get("BROWSER_RELAY_PROXY_SERVER", "").strip()
...
def find_chrome():
    ...
    if env_bin and Path(env_bin).exists():
        return env_bin
    if Path(DEFAULT_CHROME_BIN).exists():
...
class BrowserRelay:
    ...
    async def start_browser(self):
        ...
        if _PROXY_SERVER:
            args.insert(-1, f"--proxy-server={_PROXY_SERVER}")
        if sys.platform.startswith("linux"):
...
async def main():
    ...
    async with serve(
        relay.handle_client, DEFAULT_HOST, DEFAULT_PORT,
        process_request=process_request,
    ):
        print(f"服务已启动: http://{DEFAULT_HOST}:{DEFAULT_PORT}")
