...
class StateSendIntegrationTests(unittest.TestCase):
  ...
  def test_cold_full_snapshot_then_hot_omits_unchanged_lights(self):
    ...
    text_hot1, mode_hot1 = format_state_for_send(send_cold, send_hot1, is_cold=False)
    ...
    text_hot2, _ = format_state_for_send(send_hot1, send_hot2, is_cold=False)
  ...
  def test_three_turn_sequence(self):
    ...
    t2, _ = format_state_for_send(last_send, send2, is_cold=False)
    self.assertEqual(t2, '')
    last_send = send2
  # snapshot always raw
    ...
    t3, _ = format_state_for_send(last_send, send3, is_cold=False)
    self.assertEqual(t3, '')
