...
function onFile(e){
  ...
  reader.onload=function(ev){ document.getElementById('thumb').src=ev.target.result; document.getElementById('img-strip').style.display='block'; };
  reader.readAsDataURL(f); e.target.value='';
}
function removeImg(){ pendingFile=null; document.getElementById('img-strip').style.display='none'; document.getElementById('thumb').src=''; }
  ...
  try{
    ...
    if(fromHayana){
      ...
      try{
        ...
        catch(se){ console.warn('stream failed, fallback', se);
...
if('serviceWorker' in navigator){ navigator.serviceWorker.register('/sw.js').then(r=>{swReg=r;}).catch(console.error); }
