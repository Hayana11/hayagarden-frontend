// ── chat-core.js: main chat application logic ──
...
function togglePanel(id, btnId) {
  ...
  var el=document.getElementById(id), btn=document.getElementById(btnId);
  el.classList.toggle('show'); btn.classList.toggle('on', el.classList.contains('show'));
}
...
async function openHistory() {
  ...
  var r=await fetch('/api/chat/messages?limit=500'); var d=await r.json();
  allHist=(d.messages||[]).reverse(); renderHist(allHist);
}
...
function fmtTime(dt){if(!dt)return'';try{var d=new Date(dt.replace(' ','T'));return String(d.getHours()).padStart(2,'0')+':'+String(d.getMinutes()).padStart(2,'0');}catch(e){return'';}}
...
function dayLabel(dt){if(!dt)return'';try{var d=new Date(dt.replace(' ','T'));var t=new Date();var y=new Date();y.setDate(t.getDate()-1);if(d.toDateString()===t.toDateString())return'Today';if(d.toDateString()===y.toDateString())return'Yesterday';return['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][d.getMonth()]+' '+d.getDate();}catch(e){return'';}}
...
function buildToolCardHtml(tc){
  var label=TOOL_LABELS[tc.name]||tc.name;var status=tc.success?'· 成功':'· 失败';
...
function toggleThink(btn){
  var body=btn.nextElementSibling;var isOpen=body.classList.contains('open');
  body.classList.toggle('open');btn.classList.toggle('open');
  var lbl=btn.querySelector('.tl');if(lbl)lbl.textContent=isOpen?'▶ thinking':'▼ thinking';
