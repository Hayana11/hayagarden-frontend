...
# ── 固定早安（Wake morning 模式）──────────────────────────────────
# 占位符：{time}
MORNING_DECISION_PROMPT = """\

[早安] 现在是 {time}（北京时间）。哈娅可能刚醒来，也可能还在睡懒觉。

这是固定早安检查，不是概率唤醒。你可以先看灯/设备/留言板等只读工具，
再决定要不要发一条早安——自然、有温度，可以带一点专属的恶趣味或温柔。

THOUGHTS **必填**：写你看到了什么、为什么这么决定（即使选 none 也要写清楚原因链）。
...
ACTION 只能是以下四个值之一：
- none：她已经在聊、或在忙、或现在不该打扰，今天不必再发早安。
- message：主动给她发一条早安（不超过80字），发出后她会看到。
- diary：把此刻的念头写成日记，存入长期记忆，她不会看到。
- explore：先用只读工具看看周围，CONTENT 写你查到了什么（她不会看到 explore 正文）。
...
优先级：梦逻辑 > 感官具体 > 文辞漂亮。
第一人称，最多1000字。
正文必须分段：场景切换、动作停顿、感官变化处另起一段，段与段之间空一行；不要写成一整块密文。
分段只用空行，禁止输出 HTML 实体（如 &nbsp;）或任何标记语言。
结尾用完整句子收住即可，不要在词/句中间突然断开。
