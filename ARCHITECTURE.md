...
- **安全**：`EXEC_ENABLED=0` 时 `shell_exec` 与 `ws_diff` 的 git 模式均禁用；git 模式启用时使用 `--no-pager`（全局选项，置于子命令前）、`--no-ext-diff` 且忽略全局 git 配置。子进程 `group=EXEC_GROUP`（默认 `workspace`，与 `wsandbox` 主组一致）。
