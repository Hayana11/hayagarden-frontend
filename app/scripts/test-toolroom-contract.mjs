...
assert.match(screen, /window\.setInterval\(refresh, 10_000\)/);
assert.match(screen, /document\.visibilityState === 'visible'/);
assert.match(screen, /loadInventory\(false\)/);
assert.match(screen, /groupId\.startsWith\('external_mcp:'\)/);
assert.match(screen, /group\.lifecycle_state/);
...
assert.match(screen, /新增后会立即检查连接。连接成功后即可使用发现到的工具。/);
assert.doesNotMatch(screen, /等待审核|需要审核|approval|REVIEW_REQUIRED/i);
...
assert.match(screen, /preserveAspectRatio="xMidYMid meet"/);
assert.match(css, /\.toolroom-mcp-shell\s*\{[\s\S]*?overflow:\s*hidden/);
assert.match(css, /\.toolroom-tabs\s*\{[\s\S]*?align-self:\s*center/);
assert.doesNotMatch(screen, /name="description"|externalMcpForm\.description/);
...
assert.match(css, /\.toolroom-mcp-shell\s*\{[\s\S]*?height:\s*620px[\s\S]*?max-height:\s*calc\(100% - 24px\)/);
assert.match(css, /@media \(max-width: 360px\)[\s\S]*?\.toolroom-mcp-shell\s*\{[\s\S]*?height:\s*620px[\s\S]*?max-height:\s*calc\(100vh - 32px\)/);
assert.match(css, /\.toolroom-mcp-dialog::[-]webkit-scrollbar\s*\{[\s\S]*?display:\s*none/);
assert.match(css, /\.toolroom-mcp-shell::before\s*\{[\s\S]*?top:\s*11px[\s\S]*?right:\s*11px[\s\S]*?bottom:\s*11px[\s\S]*?left:\s*11px[\s\S]*?border:/);
assert.match(css, /\.toolroom-mcp-shell\s*\{[\s\S]*?max-width:\s*380px/);
...
assert.match(screen, /M20 12a8 8 0 0 0-13\.66-5\.66L4 8\.68/);
assert.match(screen, /M20 19v-3\.68h-3\.68/);
assert.match(screen, /externalMcpVisualForGroup/);
assert.match(css, /\.toolroom-mcp-check-button\s*\{[\s\S]*?display:\s*flex[\s\S]*?align-items:\s*center[\s\S]*?justify-content:\s*center/);
assert.doesNotMatch(css, /\.toolroom-mcp-check-button\s*\{[\s\S]*?place-items/);
assert.match(css, /\.toolroom-mcp-check-icon\s*\{[\s\S]*?display:\s*block/);
assert.match(screen, /'--toolroom-group-icon-background'/);
assert.match(screen, /title=\{checkLabel\}/);
assert.match(css, /--toolroom-group-icon-background/);
assert.match(css, /--toolroom-group-icon-color/);
