...
assert.match(screen, /<Link data-testid="chat-preview-entry" to=\{ROUTES\.manualContextWindow\} onClick=\{\(\) => setSidebarOpen\(false\)\}/);
assert.match(screen, />Preview<\/span>/);
assert.match(screen, /Manual window · 状态预览/);
