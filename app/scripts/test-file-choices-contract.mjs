...
assert.doesNotMatch(screen, /data-testid="chat-preview-entry"/);
assert.doesNotMatch(screen, /href="https:\/\/love-style\.xyz\/preview\/"/);
assert.doesNotMatch(screen, />Preview<\/span>/);
assert.doesNotMatch(screen, /Chat 测试页 · 实时预览/);
assert.doesNotMatch(screen, /to="\/contacts"/);
assert.doesNotMatch(screen, />通讯录<\/span>/);
assert.doesNotMatch(screen, /to="\/group-chat"/);
assert.doesNotMatch(screen, />群聊房间<\/span>/);
assert.match(screen, />系统配置<\/span>/);
assert.match(screen, /to="\/settings"[^>]*linear-gradient\(135deg,rgba\(226,218,238,\.72\),rgba\(249,228,232,\.76\)\)/);
