...
CAPABILITY_MANIFEST: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "side_effect": "external_state",
        "autonomy_mode": "explicit_or_ask",
        "trigger": "用户明确要求在未来一段时间后主动联系，或明确授权安排一次稍后联系时。",
        "purpose": "通过既有 self_triggers 生命周期安排一次未来主动联系，不创建行动计时、日期倒数、待办或日历事件。",
        "deny_when": "分钟数不是 1-1440 的整数，或用户没有明确要求且尚未完成现有 ASK 授权合同时。",
        "failure_behavior": "明确说明没有安排稍后联系；不得伪造 trigger id 或 trigger_at，也不得退化为 task.timer、countdown、todo 或 calendar。",
        "loading_policy": "deferred",
        "provider_bindings": {"claude_code": "mcp__capability__self_trigger_schedule"},
    },
    {
        "capability_id": "self_trigger.cancel",
        "display_name": "取消稍后联系",
        "kind": "write",
        "side_effect": "external_state",
        "autonomy_mode": "explicit_or_ask",
        "trigger": "用户明确要求取消一条已知的稍后联系，并提供该联系的整数 id 时。",
        "purpose": "通过既有 self_triggers 取消接口取消指定的一次未来主动联系。",
        "deny_when": "id 缺失、不是正整数，或用户只表达模糊取消意图而没有提供具体 id 时。",
        "failure_behavior": "明确说明没有取消稍后联系；不得省略 id、猜测 id 或取消全部未触发联系。",
        "loading_policy": "deferred",
