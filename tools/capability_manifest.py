...
CAPABILITY_MANIFEST: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "display_name": "查看健康数据连接状态",
        "kind": "read",
        "side_effect": "none",
        "autonomy_mode": "read_auto",
        "trigger": "用户询问健康数据连接状态，或读取健康数据前需要确认连接状态时。",
        "purpose": "返回 Xiaomi Fitness Cloud 只读 provider 的连接与鉴权状态，不返回任何凭证或账号标识。",
        "deny_when": "当前话题与健康数据连接无关时。",
        "failure_behavior": "说明连接状态不可用，不得猜测或暴露凭证。",
        "loading_policy": "deferred",
        "provider_bindings": {"claude_code": "mcp__internal__health_status", "internal_mcp": "mcp__internal__health_status"},
    },
    {
        "capability_id": "health.latest",
        "display_name": "读取最近健康摘要",
        "kind": "read",
        "side_effect": "none",
        "autonomy_mode": "read_auto",
        "trigger": "用户询问最近的步数、睡眠或心率数据时。",
        "purpose": "从 Xiaomi Fitness Cloud 读取最近的步数、睡眠和心率摘要。",
        "deny_when": "当前话题与个人健康数据无关时。",
        "failure_behavior": "说明数据读取失败或为空；缓存数据必须标示 cached/stale，不得伪装为实时数据。",
        "loading_policy": "deferred",
        "provider_bindings": {"claude_code": "mcp__internal__health_latest", "internal_mcp": "mcp__internal__health_latest"},
    },
    {
        "capability_id": "health.steps",
        "display_name": "读取每日步数",
        "kind": "read",
        "side_effect": "none",
        "autonomy_mode": "read_auto",
        "trigger": "用户询问本人近期步数或步行活动时。",
        "purpose": "读取 Xiaomi Fitness Cloud 的每日步数，days 限定为 1 到 30 天。",
        "deny_when": "当前话题与本人步数无关时。",
        "failure_behavior": "说明数据读取失败或为空；不得猜测步数。",
        "loading_policy": "deferred",
        "provider_bindings": {"claude_code": "mcp__internal__health_steps", "internal_mcp": "mcp__internal__health_steps"},
    },
    {
        "capability_id": "health.sleep",
        "display_name": "读取每日睡眠",
        "kind": "read",
        "side_effect": "none",
        "autonomy_mode": "read_auto",
        "trigger": "用户询问本人近期睡眠记录时。",
        "purpose": "读取 Xiaomi Fitness Cloud 的每日睡眠记录，days 限定为 1 到 30 天。",
        "deny_when": "当前话题与本人睡眠无关时。",
        "failure_behavior": "说明数据读取失败或为空；不得推测睡眠时长。",
        "loading_policy": "deferred",
        "provider_bindings": {"claude_code": "mcp__internal__health_sleep", "internal_mcp": "mcp__internal__health_sleep"},
    },
    {
        "capability_id": "health.heart_rate",
        "display_name": "读取每日心率",
        "kind": "read",
        "side_effect": "none",
        "autonomy_mode": "read_auto",
        "trigger": "用户询问本人近期心率记录时。",
        "purpose": "读取 Xiaomi Fitness Cloud 的每日心率记录，days 限定为 1 到 30 天。",
        "deny_when": "当前话题与本人心率无关时。",
        "failure_behavior": "说明数据读取失败或为空；不得猜测心率。",
        "loading_policy": "deferred",
        "provider_bindings": {"claude_code": "mcp__internal__health_heart_rate", "internal_mcp": "mcp__internal__health_heart_rate"},
    },
    {
