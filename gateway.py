...
def _cc_resident_stream_gen(
    ...
    if is_cold:
        ...
        def _assemble_cold_content(msgs):
            ...
            if prepared_msgs:
                ...
                if (
                    ...
                    and authored_thinking_instruction_suffix(
                        display_thinking_mode, display_thinking_prompt,
                    )
                ):
        ...
        content = append_authored_thinking_instruction(
            content, display_thinking_mode, display_thinking_prompt,
        )
        commit_meta = {
            'state_snapshot': raw_state,
            'feedback_ids': list(one_shot.get('feedback_ids') or []),
...
def _stream_cc_daily_soft_window(
    ...
    try:
        ...
        _display_thinking_mode, _display_thinking_prompt = get_display_thinking_snapshot()
        ...
        prepare_daily_display_thinking_plan(
            _daily_plan, _display_thinking_mode, _display_thinking_prompt,
        )
        ...
        try:
            ...
            if _fallback_provider == 'deepseek':
                ...
                try:
                    ...
                    _ds_model = str(config_store.get('DEEPSEEK_CHAT_MODEL', 'deepseek-flash') or '').strip() or 'deepseek-flash'
                    _ds_pay = {'model': _ds_model, 'max_tokens': 8000, 'messages': _ds_msgs}
        ...
        if _get_provider() == 'claude_code':
            ...
            try:
                ...
                if _he.code in (401, 403, 503):
                    ...
                    _ds_model = str(config_store.get('DEEPSEEK_CHAT_MODEL', 'deepseek-flash') or '').strip() or 'deepseek-flash'
                    _ds_payload = {
                        'model': _ds_model, 'max_tokens': 500,
                        'messages': [
